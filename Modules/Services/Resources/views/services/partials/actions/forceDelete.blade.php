@if(auth()->user()->hasPermission('forceDelete_services'))
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"
                        id="modal-title-{{ $service->id }}">@lang('services::services.dialogs.force.title')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    @lang('services::services.dialogs.force.info')
                </div>
                <div class="modal-footer">
                    {{ BsForm::get(route('dashboard.services.forceDelete', $service)) }}
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">
                        @lang('services::services.dialogs.force.cancel')
                    </button>
                    <button type="submit" class="btn btn-danger">
                        @lang('services::services.dialogs.force.confirm')
                    </button>
                    {{ BsForm::close() }}
                </div>
            </div>
